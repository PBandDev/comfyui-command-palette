...
const EXAMPLE_NODE_ID = ["Template", "Example", "NormalizeText"].join("");

test("frontend-only command palette extension loads in ComfyUI", async ({ page, request }) => {
  await page.goto("/");
  ...
  await expect(page.locator('[data-command-palette="styles"]')).toBeAttached();
  ...
  expect(objectInfo[EXAMPLE_NODE_ID]).toBeUndefined();
});
