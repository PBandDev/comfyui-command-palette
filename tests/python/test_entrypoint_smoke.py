...
def test_frontend_only_entrypoint_exports_expected_symbols_via_package_loader():
    module = load_package_from_path(
        "command_palette_entrypoint",
        ENTRYPOINT_PATH,
        repo_root=REPO_ROOT,
    )
    ...
    assert not hasattr(module, "Example" + "NormalizeTextNode")


def test_registry_icon_is_release_specific():
    icon = ICON_PATH.read_text(encoding="utf-8")

    assert "Replace this default " + "icon" not in icon
    assert "Template" not in icon
    assert 'aria-label="Command key"' in icon
    assert "keycapFace" in icon
    assert "M15 6v12" in icon
...
def test_comfyignore_excludes_release_unwanted_artifacts():
    required_entries = {
        ".agents/",
        ".github/",
        "tests/",
    ...
    assert COMFYIGNORE_PATH.is_file()
    ...
    assert required_entries <= comfyignore_entries
...
def test_ci_workflows_use_repo_command_surface():
    ...
    assert "Comfy-Org/publish-node-action@main" in publish_workflow
